...
parser.add_argument('--out_json_fname', type=str, default='CPU_perf_singlethread_stripes.json')
parser.add_argument('--overwrite', type=int, default=0, help="0: skip existing results, 1: overwrite")
parser.add_argument('--cpp_exe_dir', type=str, default='/mnt/scratch/wenqi/spatial-join-baseline/cpp/1d_stripes', help="the CPP exe file")
...
parser.add_argument('--log_name', type=str, default='cpu_st_stripe_log', help="output log")
...
if args.num_partitions is None:
    num_partitions_list = [100, 1000, 10000, 100000] # larger sizes are very slow for single-thread sync-traversal
else:
...
def parse_perf_result(fname):
    ...
    Return: num_results, partition_ms, build_index_2_ms, join_ms, bfs_static_ms, bfs_dfs_static_ms, bfs_dynamic_ms, bfs_dfs_dynamic_ms
    """
...
def config_exist_in_dict(json_dict, dataset, join_type, size_dataset_A, size_dataset_B, num_partitions):
    ...
    if num_partitions not in json_dict[dataset][join_type][size_dataset_A][size_dataset_B]:
        return False
    if "p0s1_num_results" in json_dict[dataset][join_type][size_dataset_A][size_dataset_B][num_partitions] and \
        "p0s1_partition_ms" in json_dict[dataset][join_type][size_dataset_A][size_dataset_B][num_partitions] and \
        "p0s1_join_ms" in json_dict[dataset][join_type][size_dataset_A][size_dataset_B][num_partitions] and \
        "p1s0_num_results" in json_dict[dataset][join_type][size_dataset_A][size_dataset_B][num_partitions] and \
        "p1s0_partition_ms" in json_dict[dataset][join_type][size_dataset_A][size_dataset_B][num_partitions] and \
        "p1s0_join_ms" in json_dict[dataset][join_type][size_dataset_A][size_dataset_B][num_partitions] and \
        "best_join_direction" in json_dict[dataset][join_type][size_dataset_A][size_dataset_B][num_partitions] and \
        "best_join_ms" in json_dict[dataset][join_type][size_dataset_A][size_dataset_B][num_partitions]:
        return True
    else:
        return False
...
def write_json(fname, overwrite, json_dict, 
            ...
            array_p0s1_num_results, array_p0s1_partition_ms, array_p0s1_join_ms,
            array_p1s0_num_results, array_p1s0_partition_ms, array_p1s0_join_ms,
    ...
    if not overwrite and config_exist_in_dict(json_dict, dataset, join_type, size_dataset_A, size_dataset_B, num_partitions):
        return 
    else:
        json_dict[dataset][join_type][size_dataset_A][size_dataset_B][num_partitions]["p0s1_num_results"] = array_p0s1_num_results
        json_dict[dataset][join_type][size_dataset_A][size_dataset_B][num_partitions]["p0s1_partition_ms"] = array_p0s1_partition_ms
        json_dict[dataset][join_type][size_dataset_A][size_dataset_B][num_partitions]["p0s1_join_ms"] = array_p0s1_join_ms
        json_dict[dataset][join_type][size_dataset_A][size_dataset_B][num_partitions]["p1s0_num_results"] = array_p1s0_num_results
        json_dict[dataset][join_type][size_dataset_A][size_dataset_B][num_partitions]["p1s0_partition_ms"] = array_p1s0_partition_ms
        json_dict[dataset][join_type][size_dataset_A][size_dataset_B][num_partitions]["p1s0_join_ms"] = array_p1s0_join_ms
        json_dict[dataset][join_type][size_dataset_A][size_dataset_B][num_partitions]["best_join_direction"] = best_join_direction
        json_dict[dataset][join_type][size_dataset_A][size_dataset_B][num_partitions]["best_join_ms"] = best_join_ms
        ...
        for size_dataset_A in dataset_sizes:
            for size_dataset_B in dataset_sizes:
                for num_partitions in num_partitions_list:
                    ...
                    for reps in range(num_runs):
                        ...
                        if dataset == 'Uniform':
                            if join_type == "Point-in-Polygon":
                                cmd = f'{cpp_exe_dir} {C_file_dir}/C_uniform_{size_dataset_A}_point_file_0.txt {C_file_dir}/C_uniform_{size_dataset_B}_polygon_file_0_set_0.txt {num_partitions} > {log_name}'
                            elif join_type == "Polygon-Polygon":
                                cmd = f'{cpp_exe_dir} {C_file_dir}/C_uniform_{size_dataset_A}_polygon_file_0_set_0.txt {C_file_dir}/C_uniform_{size_dataset_B}_polygon_file_1_set_0.txt {num_partitions} > {log_name}'
                        elif dataset == 'OSM':
                            if join_type == "Point-in-Polygon":
                                cmd = f'{cpp_exe_dir} {C_file_dir}/C_OSM_{size_dataset_A}_point_file_0.txt {C_file_dir}/C_OSM_{size_dataset_B}_polygon_file_0.txt {num_partitions} > {log_name}'
                            elif join_type == "Polygon-Polygon":
                                cmd = f'{cpp_exe_dir} {C_file_dir}/C_OSM_{size_dataset_A}_polygon_file_0.txt {C_file_dir}/C_OSM_{size_dataset_B}_polygon_file_1.txt {num_partitions} > {log_name}'
                    ...
                    print("------------------------------------------------------")
                    print("Finished: ", dataset, join_type, size_dataset_A, size_dataset_B, num_partitions)
                    print("------------------------------------------------------")
