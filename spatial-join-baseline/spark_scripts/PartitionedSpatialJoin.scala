...
object PartitionedSpatialJoin {
  ...
  def apply(sc: SparkContext,
    ...
    if (paritionConf.paritionMethod == PartitionMethod.FGP) {
      ...
      println("partition time (ms): " + runtime)
      ...
      rightPairs.collect()
      leftPairs.collect()
      val indexEnd = System.currentTimeMillis() - indexTime
      println("index building (+ assign partition ids) (ms): " + indexEnd)
    ...
    if (nestedLoop) {
      ...
      if (joinPredicate == SpatialOperator.NearestD) {
        ...
        val joinedPairs = leftPairs.leftOuterJoin(rightPairs).flatMap(x =>
          ...
          .distinct()
        val joinEnd = System.currentTimeMillis() - joinTime
        println("Join time (partition file): " + joinEnd)
        joinedPairs
      }
    }
