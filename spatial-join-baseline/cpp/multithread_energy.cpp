...
//   fix_tree(root_B);
  ...
  std::cout << "Entering a while loop to do the spatial join, the energy consumption can be measured simultaneously..." << std::endl;
  int loop_count = 0;

  while (true) {
	std::cout << "while loop iteration: " << loop_count++ << std::endl;

  	///// BFS + static /////
	if(strcmp(parallel_approach, "bfs_static") == 0) {
	...
	///// BFS-DFS + static /////
	if(strcmp(parallel_approach, "bfs_dfs_static") == 0) {
		omp_set_schedule(omp_sched_static, -1);  
	...
	///// BFS + dynamic /////
	if(strcmp(parallel_approach, "bfs_dynamic") == 0) {
	...
	///// BFS-DFS + dynamic /////
	if(strcmp(parallel_approach, "bfs_dfs_dynamic") == 0) {
		omp_set_schedule(omp_sched_dynamic, -1);  
