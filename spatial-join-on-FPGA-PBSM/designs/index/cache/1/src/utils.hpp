...
std::vector<int, aligned_allocator<int> > read_from_binary_file(const std::string& file_path, int& num_objects_A) {
    ...
    char header[64];
    file.read(header, 64);
    num_objects_A = *reinterpret_cast<int*>(header);
...
// read tree data
std::vector<int, aligned_allocator<int> > read_indexed_dataset(
    ...
    tree_bytes = file.tellg();
    file.seekg(0, file.beg);

    // vector-buffer
    std::vector<int, aligned_allocator<int>> buffer(tree_bytes / sizeof(int), 0);

    file.read((char*) buffer.data(), tree_bytes);

    return buffer;
}

void print_dataset(const std::vector<int, aligned_allocator<int>>& dataset_a) {
    std::cout << "CONTENT: [";

    for (size_t i = 0; i < dataset_a.size(); i++) {
        std::cout << dataset_a[i];
        if (i < dataset_a.size() - 1) std::cout << ", ";
    }
